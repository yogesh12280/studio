...
import type { Notification, User, Comment } from '@/lib/types'
...
export function NotificationCard({ notification, onLikeToggle, onDelete, onAddComment, onEditNotification, onAddReply, currentUser }: NotificationCardProps) {
  ...
  const isLiked = notification.likedBy.includes(currentUser.id)
  ...
  const likers = notification.likedBy
    .map(userId => users.find(u => u.id === userId))
    .filter((u): u is User => !!u);
  
  const viewers = (notification.viewedBy || [])
    .map(userId => users.find(u => u.id === userId))
    .filter((u): u is User => !!u);
  ...
  const handleCommentSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (newComment.trim()) {
      onAddComment(notification.id, newComment.trim())
      setNewComment('')
    }
  }
  ...
  return (
    <Card className="max-w-2xl mx-auto overflow-hidden">
      <CardHeader className="flex flex-row items-start gap-4 p-4">
        ...
        <div className="flex-1">
          <div className="flex items-center justify-between">
            <div>
              <p className="font-semibold">{notification.author.name}</p>
      ...
      <CardContent className="p-4 pt-0">
        <h2 className="text-xl font-bold font-headline mb-2">{notification.title}</h2>
          ...
          <Popover open={likePopoverOpen} onOpenChange={setLikePopoverOpen}>
            <PopoverTrigger asChild>
              <Button 
                variant="ghost" 
                size="sm" 
                onClick={() => onLikeToggle(notification.id)}
        ...
        <Popover open={viewPopoverOpen} onOpenChange={setViewPopoverOpen}>
          <PopoverTrigger asChild>
            ...
            >
              ...
              <span>{notification.viewers} views</span>
            </Button>
          </PopoverTrigger>
          {viewers.length > 0 && (
              <PopoverContent className="w-auto max-w-xs">
